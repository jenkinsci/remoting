...
set-base-image() {
    ...
    if [[ $arch == amd64 ]]; then
        BASEIMAGE="openjdk:${jdk}-jdk"
    elif [[ $arch == arm ]]; then
        BASEIMAGE="arm32v7/openjdk:${jdk}-jdk"
    elif [[ $arch == arm64 ]]; then
        BASEIMAGE="arm64v8/openjdk:${jdk}-jdk"
    elif [[ $arch == s390x ]]; then
        BASEIMAGE="s390x/openjdk:${jdk}-jdk"
    elif [[ $arch == ppc64le ]]; then
        BASEIMAGE="ppc64le/openjdk:${jdk}-jdk"
    fi
    ...
    if [ "$(uname)" == 'Darwin' ]; then
        #TODO: this code is awful and needs to be refactored ASAP before somebody sees it
        sed "s|BASEIMAGE|${BASEIMAGE}|g" "$dockerfile" > "$dockerfile-tmp"
        cp "$dockerfile-tmp" "$dockerfile"
        if [[ "${arch}" == "amd64" ]]; then
            sed "/CROSS_BUILD_/d" "$dockerfile" > "$dockerfile-tmp"
            cp "$dockerfile-tmp" "$dockerfile"
        else
            if [[ "${arch}" == "arm64" ]]; then
                sed "s|ARCH|aarch64|g" "$dockerfile" > "$dockerfile-tmp"
                cp "$dockerfile-tmp" "$dockerfile"
            else
                sed "s|ARCH|${arch}|g" "$dockerfile" > "$dockerfile-tmp"
                cp "$dockerfile-tmp" "$dockerfile"
            fi
            sed "s/CROSS_BUILD_//g" "$dockerfile" > "$dockerfile-tmp"
            cp "$dockerfile-tmp" "$dockerfile"
        fi
    else
...
get-manifest() {
    ...
    curl $opts -q -fsSL -H "Accept: application/vnd.docker.distribution.manifest.v2+json" -H "Authorization: Bearer $TOKEN" "https://index.docker.io/v2/jenkins4eval/remoting/manifests/$tag"
}
...
publish() {
    ...
    for jdk in ${JDKS[*]}; do
    for arch in ${ARCHS[*]}; do
        set-base-image "$variant" "$arch" "$jdk"
        # TODO: REMOTING_SHA is actually not used
        docker build --file "multiarch/Dockerfile$variant-$arch" \
                     --build-arg "REMOTING_VERSION=$version" \
                     --build-arg "REMOTING_SHA=$sha" \
...
tag-and-push() {
    ...
    local digest_source
    local digest_target
    local effectiveTag="${source}"
    local effectiveTarget="${target}"

    if [ "$arch" != "amd64" ] ; then
        effectiveTag="${effectiveTag}-${arch}"
        effectiveTarget="${effectiveTarget}-${arch}"
    fi

    if [ "$jdk" != 8 ] ; then
        effectiveTag="${effectiveTag}-jdk${jdk}"
        effectiveTarget="${effectiveTarget}-jdk${jdk}"
    fi

    if [ "$debug" = true ]; then
        >&2 echo "DEBUG: Getting digest for ${effectiveTag}"
    fi

    # if tag doesn't exist yet, ie. dry run
    if ! digest_source=$(get-digest "${effectiveTag}"); then
        echo "Unable to get source digest for ${effectiveTag} ${digest_source}"
        digest_source=""
    fi
    ...
    if ! digest_target=$(get-digest "${effectiveTag}"); then
        echo "Unable to get target digest for ${effectiveTag} ${digest_target}"
        digest_target=""
    fi

    if [ "$digest_source" == "$digest_target" ] && [ -n "${digest_target}" ]; then
        echo "Images ${effectiveTag} [$digest_source] and ${effectiveTag} [$digest_target] are already the same, not updating tags"
    else
        echo "Creating tag ${effectiveTarget} pointing to ${effectiveTag}"
        #docker-tag "${source}-${arch}" "jenkins" "${target}-${arch}"
        docker-tag "${effectiveTag}" "$REGISTRY" "${effectiveTarget}"

        if [ ! "$dry_run" = true ]; then
            echo "Pushing $REGISTRY/$IMAGE:${effectiveTarget}"
            docker push "$REGISTRY/$IMAGE:${effectiveTarget}"
        else
            echo "Would push $REGISTRY/$IMAGE:${effectiveTarget}"
        fi
    fi
}
...
publish-variant() {
    ...
    for arch in ${ARCHS[*]}; do
        if [[ "$variant" =~ slim ]]; then
            tag-and-push "${version}${variant}" "slim" "${arch}" "${jdk}"
        elif [[ "$variant" =~ alpine ]]; then
            tag-and-push "${version}${variant}" "alpine" "${arch}" "${jdk}"
        fi
    done
    done
...
publish-latest() {
    ...
    for arch in ${ARCHS[*]}; do
        ...
        if [ -z "$variant" ]; then
            tag-and-push "${version}${variant}" "latest" "${arch}" "${jdk}"
        fi
    done
    done
...
publish-lts() {
    ...
    for jdk in ${JDKS[*]}; do
    for arch in ${ARCHS[*]}; do
        tag-and-push "${version}${variant}" "lts${variant}" "${arch}" "${jdk}"
    done
    done
...
push-manifest() {
    ...
    # TODO: not pushing manifests now, single platform
